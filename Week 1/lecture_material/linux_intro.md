...
1. Enter `cd <name-of-one-of-the-dirs-seen>`
...
1. Enter `mkdir Test`. Enter `ls` to verify you now see a directory called test

`mkdir` is used to make new directories

1. Enter `rm -rf Test` or `rmdir` to remove `Test`. Enter `ls` to verify your changes
...
1. Navigate to directory containing a file and enter `cat <FILENAME>`

`cat` outputs the contents of a file to your screen

1. Create a new text file with `touch test.txt`

1. Run `echo 'Testing' > test.txt`. Verify the new text is present
...
1. Run `rm test.txt`. Verify it has been deleted
...
1. Try to install terminator (a bash shell you are likely using) with `apt-get install terminator`. **This will fail since you are not root!**

1. Give yourself greater permissions by entering `sudo apt-get install terminator` instead
